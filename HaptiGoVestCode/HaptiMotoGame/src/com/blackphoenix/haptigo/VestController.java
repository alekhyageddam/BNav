...
public class VestController 
{
	...
	public static final int SHORT_BUZZ = 400,
			LONG_BUZZ = 600,
			BUZZ_OFF_TIME = 150;

	// constants that define the amplitude of vibes and index of tactors on vest 
	public static final int VIBE_HIGH = 255,
			VIBE_LOW = 0;
	
	public static final int VIBE_LEFT_INDEX = 0,
			VIBE_RIGHT_INDEX = 2,
			VIBE_CENTER_INDEX = 1;
	
	
	private static final int NUM_PULSES_INDEX = 4,
			BUZZ_LENGTH_INDEX = 3;
	...
	//sending message to vest to reset the vibration motors
	public static void resetVibrationIntensities(){
		Amarino.sendDataToArduino(context, deviceAddress, 'r', 0);
	}
	
	//Sending message to set the vest to light mode
	public static void setLightMode(){
		Amarino.sendDataToArduino(context, deviceAddress, 'l', 0);
	}
	
	//Sending message to set the vest to vibe mode
	public static void setVibeMode(){
